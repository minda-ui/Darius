# Charter version history — `CLAUDE.md`

**The `Changed in vNN` notes from the Workshop of Furniture Making charter, newest first.** Split out of
`CLAUDE.md` at **v27** (2026-09-21) because the charter had reached 110,025 bytes and Drive has no patch
API — every version costs a full re-emission, and the file had grown past the largest transfer this KB
had verified. See the v27 note in `CLAUDE.md`, and `HL-0030` on the Workforce Hub for the estate-wide
version of the question.

**Nothing here was rewritten.** The notes below are the bytes that were in `CLAUDE.md`, moved unchanged.
